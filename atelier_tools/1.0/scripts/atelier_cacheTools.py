...
def atelier_ToolsUI():
    win = cmds.window(t='Atelier Cache Tools', s=True)
    # CREATE UI PIECES
    frb = cmds.formLayout()
...
def mCacheUI(parent):
    ...
    cmds.button(
        bs, e=True,
        c='cmds.textScrollList("{0}", e=True, ra=True);cmds.textScrollList("{0}", e=True, a=atelier_cacheTools.getExportSET(cmds.optionMenu("{1}", q=True, v=True))[0])'.format(ts, oms))
    # LIST UI
