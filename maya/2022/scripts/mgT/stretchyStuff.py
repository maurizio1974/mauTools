    ...
    for eIK in iks:
        if cmds.objectType( eIK ) == 'ikHandle':
            if cmds.ikHandle( eIK ,q = True, sol = True ) != 'ikSplineSolver':
                ...
                DD = cmds.shadingNode( asUtility = 'distanceBetween', n = (name + 'DD') )
                DDS = cmds.shadingNode( asUtility = 'distanceBetween', n = (name + 'DDS') )

                # CONNECTED TO THE RESPECTIVE LOCATOR PREVIOUSLY CREATED
                # CONNECTION FOR THE DISTANCE USED FOR THE STRETCH
                cmds.connectAttr( (sLoc[0] + 'Shape.worldPosition[0]'), (DD + '.point1') )
                cmds.connectAttr( (eLoc[0] + 'Shape.worldPosition[0]'), (DD + '.point2') )

                # CONNECTION FOR THE DISTANCE THAT WILL TELL US THE WORD SCALE OF THE RIG
                connectAttr( (sLocS[0]+'Shape.worldPosition[0]'), (DDS + '.point1') )
                connectAttr( (eLocS[0]+'Shape.worldPosition[0]'), (DDS + '.point2') )
                ...
                connectAttr( (CND + '.outColorR'), (MD+'.input1X') )
                connectAttr( (DDS + '.distance'), (MD+'.input2X') )
                ...
                for eChain in chain:
                    if cmds.objectType( eChain ) == 'joint':
                        cmds.connectAttr( (MD + '.outputX' ), (eChain + '.scaleX') )
                ...
                for eC in conn:
                    ...
                    if len( shape[0] ) != 0:
                        if cmds.nodeType( shape[0] ) == "nurbsCurve":
                            ...
                            cmds.connectAttr( (cINFO + '.arcLength'), (MD + '.input1X');
                            ...
                            for eC in  chain :
                                ...
                                if cmds.nodeType(eC) != 'ikEffector':
                                    cmds.connectAttr( ($MD+".outputX"), ($eC+".scaleX") )
        else:
