            ...
            try:
                for ai in attrsIn:
                    try:
                        ...
                        print 'Skipping ' + node1 + '.' + ao + ' --> ' + node2 + '.' + ai
                        continue
            except:
...
def nodePlusMM(ned):
    ...
    cmds.menuItem(
        label='Connector', p=nedPP, c='import json;import mnmConnectPopUP;reload(mnmConnectPopUP);mnmConnectPopUP.mnmNodeAttr(\'out\')')
    ...
    cmds.menuItem(label='CleanUP + Joints', p=nedPP, c='mnmGraphPlus.mauCleanSkinNE()')
    cmds.menuItem(
    ...
    cmds.menuItem(label='------------', p=nedPP)
    cmds.menuItem(label='Clean Separate', p=nedPP, c='mnmGraphPlus.mauSeaparate(0)')
    ...
    cmds.menuItem(label='Clean Unite + Transform', p=nedPP, c='mnmGraphPlus.mauUnite(0)')
...
def nodeEdCreateToolbarPy(panelName, fl):
    ...
    alHYPG = cmds.iconTextButton(
        ...
        c='import nodeAlignDeezUI;reload(nodeAlignDeezUI);nodeAlignDeezUI.run()'
    )
    item = ned + "LCSTATE"
    ...
    isoHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.mParentSh()'
    )
    item = ned + "LCISO"
    isoHYPG = cmds.iconTextButton(
        item,
        ...
        h=iconsize,
        ann='Isolate',
        c='reload(mnmGraphPlus);mnmGraphPlus.mauIsolate(0)'
    )
    item = ned + "LCSHAPE"
    shHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.mauCreateSh()'
    )
    ...
    connectorHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.mauAttrConnector(cmds.ls(sl=True)[0], cmds.ls(sl=True)[1])'
    )
    ...
    connectorMHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);reload(mnmConnectPopUP);mnmConnectPopUP.mnmNodeAttr("out")'
    )
    item = ned + "LCSWN"
    swapHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.swapConnections()'
    )
    item = ned + "LCSEPA"
    sapaHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.mauSeaparate(0)'
    )
    item = ned + "LCUNITE"
    uniteHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.mauUnite(1)'
    )
    item = ned + "LCGRP"
    grpHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.mauGroupCreate()'
    )
    item = ned + "LCDUP"
    dupHYPG = cmds.iconTextButton(
        ...
        c='reload(mnmGraphPlus);mnmGraphPlus.mauDupChain()'
    )
    item = ned + "LCSDC"
    ...
    pmi444 = cmds.menuItem(label='Swap first and last node selected connections and disconnet Orig', p=mi444)
    cmds.menuItem(pmi444, e=True, c='reload(mnmGraphPlus);mnmGraphPlus.swapConnections(None, None, None, True)')
