...
class PoseWrangler(ui_window.UIWindow, form_class):
    ...
    def __init__(self, parent=qt_util.maya_window()):
        ...
        if not cmds.pluginInfo('MayaUE4RBFPlugin2022', q=True, loaded=True):
            cmds.loadPlugin('MayaUE4RBFPlugin2022')
        ...
        self.driven_transforms_LIST.itemSelectionChanged.connect(
            self.driven_changed)
    ...
    def driver_popup(self, point):
        ...
        select_driver_action = QtWidgets.QAction(
            "Select Driver Transform", self)
        ...
        select_driven_action = QtWidgets.QAction(
            "Select Driven Transform(s)", self)
        select_driven_action.triggered.connect(self.select_driven)

        """--------------------------MIRROR----------------------------"""

        mirror_driver_action = QtWidgets.QAction(
            "Mirror Selected Drivers", self)
        mirror_driver_action.triggered.connect(self.mirror_driver)

        """----------------------IMPORT/EXPORT----------------------------"""

        export_driver_action = QtWidgets.QAction(
            "Export Selected Drivers", self)
        ...
        zero_base_pose_action = QtWidgets.QAction(
            "Zero Base Pose Transforms", self)
    ...
    def add_pose(self):
        ...
        if not selected_solvers:
            OpenMaya.MGlobal.displayError(
                'PoseWrangler: You must have a driver selected!')
            return
        ...
        if pose_name:
            ...
            cmds.warning(
                'PoseWrangler: You must enter a pose name to add a pose.')
    ...
    def add_new_driven(self):
        ...
        if not sl:
            OpenMaya.MGlobal.displayError(
                "No driven object selected, please select a driven object to add")
    ...
    def pose_changed(self):
        ...
        if selected_poses:
            ...
            if selected_pose in solver.pose_dict.keys():
                ...
                cmds.warning('Pose ' + selected_pose +
                             ' not found in pose dictionary')
        self.refresh_ui_state()
    ...
    def load_drivers(self, selected=None):
        ...
        if self.drivers:
            for driver in self.drivers:
                ...
                solver = poseWrangler.UE4PoseDriver(
                    existing_interpolator=driver)
    ...
    def import_drivers(self):
        ...
        file_path = QtWidgets.QFileDialog.getOpenFileName(
            self, "Pose Wrangler Format", "", "JSON (*.json)")[0]
        if file_path == "":
            return
    ...
    def _export(self, drivers):
        ...
        file_path = QtWidgets.QFileDialog.getSaveFileName(
            None, "Pose Wrangler Format", "", "*.json")[0]
        if file_path == "":
            return
    ...
    def create_driver(self):
        ...
        interp_name, ok = QtWidgets.QInputDialog.getText(
            self, 'text', 'Driver Name:')
        if interp_name:
