...
def get_selected_mesh():
    ...
    """
    sel = cmds.ls(sl=True, int=True, transforms=True)
    
    if not sel:
        shapes = cmds.ls(sl=True, int=True, objectsOnly=True)
        if shapes:
...
def get_all_vert_indexes(obj):
    ...
    """
    if is_curve(obj):
        return cmds.ls("{0}.cv[*]".format(obj), int=True, flatten=True)
    else:
        return cmds.ls("{0}.vtx[*]".format(obj), int=True, flatten=True)
...
def get_vert_indexes(obj):
    ...
    """
    if is_curve(obj):
        return cmds.ls("{0}.cv[*]".format(obj), sl=True, int=True, flatten=True)
    else:
        components = [x for x in cmds.ls(sl=True, int=True, type="float3") if x.startswith(obj)]
        return cmds.ls(cmds.polyListComponentConversion(components, toVertex=True), int=True, flatten=True)
...
def display_multi_color_influence(obj, skin_cluster, skin_data, vert_filter=[]):
    ...
    for vert_index in skin_data:
        ...
        for inf, weight in list(skin_data[vert_index]["weights"].items()):
...
def select_inf_vertexes(obj, infs, skin_data):
    ...
    for vert_index in skin_data:
        vert_infs = list(skin_data[vert_index]["weights"].keys())
...
def set_skin_weights(obj, skin_data, vert_indexes, normalize=False):
    ...
    for vert_index in vert_indexes:
        ...
        for inf_name, weight_value in list(skin_data[vert_index]["weights"].items()):
...
def average_by_neighbours(obj, vert_index, skin_data, strength):
    ...
    for index in neighbours:
        for inf, value in list(skin_data[index]["weights"].items()):
            # Ignore if locked
...
def smooth_weights(obj, vert_indexes, skin_data, strength, normalize_weights=True):
    ...
    # Set weights
    for vert_index, weights in list(weights_to_set.items()):
...
def flood_weights_to_closest(mesh, skin_cluster):
    ...
    inf_positions = {
        ...
        for key, inf in list(influences.items())
    }
    ...
    for vert_index, inf_index in list(vert_inf_mappings.items()):
