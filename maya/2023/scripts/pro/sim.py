...
def session_envs(file_id):
    ...
    for key, val in os.environ.items():
        ...
        vals = [x for x in vals if user_home not in x and '=' not in x]
        values = []
        for v in vals:
            values.append('"' + v + '"')
        if key == '_':
...
def sim(simMesh, evaluate, start, end, settings, mode):
    ...
    #     r';file -rename \"{0}\";'
    #     r'currentTime {1};'
    #     r'playbackOptions -max {2};'
    #     r'file -save -type \"{3}\";'
    #     r'$startTime = `timerX`;'
    # ).format(newScene, start, end, fileT)
    cmd += (
        r';file -rename \"{0}\";'
        r'currentTime {1};'
        r'playbackOptions -max {2};'
    ...
    if not mode:
        ...
        launcher += title + '; ' + cmd + '; exec $SHELL &\''
        # sh.os.system(
        #     'mate-terminal -t \'Shell Sim: ' + title + '\' -e \'' + cmd + '\' &'
        # )
        print(launcher)
        subprocess.Popen(["/bin/bash", "-c", launcher])
    else:
...
def mSimUI(parent):
    ...
    ppss = cmds.popupMenu('simPP', p=ts)
    cmds.menuItem(
        l=prst[0], p=ppss,
    ...
    cmds.menuItem(
        l=prst[2], p=ppss,
        c='cmds.textScrollList("{0}", e=True, ra=True)'.format(ts)
    )
    cmds.menuItem(
        l=prst[3], p=ppss,
...
def mCacheUI(parent):
    ...
    # presets for scene name
    prst = ['cached', 'test', 'mayaC', 'alembic']
    pps = cmds.popupMenu('settingsPP', p=ttv)
    for p in prst:
        cmds.menuItem(
            l=p,
