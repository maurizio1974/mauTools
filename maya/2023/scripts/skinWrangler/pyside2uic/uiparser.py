...
class UIParser(object):
    def __init__(self, QtCoreModule, QtGuiModule, QtWidgetsModule, creatorPolicy):
        ...
        self.wprops = Properties(
            self.factory, QtCoreModule, QtGuiModule, QtWidgetsModule)
    ...
    def setupObject(self, clsname, parent, branch, is_attribute=True):
        name = self.uniqueName(branch.attrib.get("name")
                               or clsname[1:].lower())
        if parent is None:
            args = ()
        else:
    ...
    def createWidget(self, elem):
        ...
        if isinstance(topwidget, QtWidgets.QToolBox):
            ...
            if icon is not None:
                topwidget.addItem(
                    widget, icon, self.wprops.getAttribute(elem, "label"))
            else:
                topwidget.addItem(
                    widget, self.wprops.getAttribute(elem, "label"))
            ...
            if icon is not None:
                topwidget.addTab(
                    widget, icon, self.wprops.getAttribute(elem, "title"))
            else:
                topwidget.addTab(
                    widget, self.wprops.getAttribute(elem, "title"))
    ...
    def handleHeaderView(self, elem, name, header):
        ...
        value = self.wprops.getAttribute(
            elem, name + "CascadingSectionResizes")
        if value is not None:
            ...
            if elem.tag == "column":
                ...
                if whatsThis:
                    w.horizontalHeaderItem(
                        self.column_counter).setWhatsThis(whatsThis)
                ...
                if whatsThis:
                    w.verticalHeaderItem(
                        self.row_counter).setWhatsThis(whatsThis)
    ...
    def createActionGroup(self, elem):
        action_group = self.setupObject(
            "QActionGroup", self.toplevelWidget, elem)
