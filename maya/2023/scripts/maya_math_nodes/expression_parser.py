...
class ExpressionParser(object):
    ...
    def parse_number(self):
        ...
        next(self._data)  # consumes number
    ...
    def parse_element(self):
        ...
        if self.token.type == NumberToken:
            ...
            if self.token.value == 'true' or self.token.value == 'false':
                ...
                next(self._data)  # consumes boolean
                return num
            ...
            next(self._data)  # consume negate
    ...
    def parse_parentheses(self):
        ...
        Returns:
            Number | String | Binary | Conditional | Function: Returns appropriate AST node
        """
        next(self._data)  # consume open paren
        ...
        next(self._data)  # consume closing paren
        return result
    ...
    def parse_function(self):
        ...
        next(self._data)  # consume function
        ...
        while True:
            ...
            if self.token and self.token.value == ']':
                ...
                if self.token and self.token.value in [',', ')']:
                    ...
                    next(self._data)  # consume comma or close paren
                ...
                next(self._data)  # consume close paren
                break
            elif self.token and self.token.value == ',':
                next(self._data)  # consume comma
            else:
                self._data.error('Expected closing parenthesis, got "{0}" instead'.format(self.token))
