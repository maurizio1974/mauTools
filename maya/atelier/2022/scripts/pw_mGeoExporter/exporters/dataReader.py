...
#        pref = self.checkLegalChar(pref)
    ...
    def reorderAttrs(self, attrs):
        ...
        for a in attrs:
            ...
            res.update({a['src']: { k:v for k, v in list(a.items()) if not k == 'src'}})
        return res
            ...
            if len(types)>1:
                ...
                if types[0] in allTypes or not attrs[a]['default']:
                    ...
                    if not attrs[a] is None:
                        df = attrs[a]['default']
                        if not df and isinstance(df, str):
                            df = defaults[types[0]]
...
#            PER SHAPE GROUPS
    ...
    def readVertexUV(self,iter):
        ...
        #                            try:
                                    ...
                                    u = list(uArray)
                                    v = list(vArray)
                                    for i in reversed(list(range(len(u)))):
...
#                                iter.getDependNode( mObj )
                            ...
                            while not iterPolys.isDone():
                                ...
                                for j in reversed( list(range( colors.length())) ):
...
#                       iter.getDagPath( dagPath )
                    ...
                    while not iterPolys.isDone():
                        ...
                        for i in reversed(list(range(nrm.length()))):
