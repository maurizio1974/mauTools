...
OUPOST SIM/CACHE UTILITY

Date = 20/08/2018
User = Maurizio Giglioli
Update =
        05/11/2018 Better handling of namespaces
        16/10/2018 Added UI Checks and switched to direct connections
        10/08/2018 Reqorked UI and commands to cache
        20/08/2018 First Release
...
def alembicWrapOutSingle(start, end, sceneN, cacheSets, out):
    ...
    for cache in cacheSets:
        ...
        # abcCurrent = item[0].replace(':', '_') + '.abc'
        ...
        cmd = (
            ...
            '-writeUVSets -uvWrite -dataFormat ogawa -stripNamespaces '
...
def getCacheLocation():
    ...
    ws = cmds.file(q=True, sn=True).replace('/scenes/' + name, '')
...
def aToolsUI():
    win = cmds.window(t='Outpost Cache Tools', s=True)
    # CREATE UI PIECES
    frb = cmds.formLayout()
...
def mCacheUI(parent):
    ...
    cmds.button(
        bs, e=True,
        c='cmds.textScrollList("{0}", e=True, ra=True);cmds.textScrollList("{0}", e=True, a=cacheTools.getExportSET(cmds.optionMenu("{1}", q=True, v=True))[0])'.format(ts, oms))
    # LIST UI
