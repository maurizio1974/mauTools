import time as Time
import re
from . import simplejson as hjson
from . import dataReader
from .imports import *
from . import templateMaker as template
import subprocess
...
######### DUILD DATA ####################################################################
    ...
    def collectData(self):
        ...
        for typeData in self.poly, self.curve, self.particles, self.pivots:
            #points attrib
            for k in list(typeData.pointAttribArray.keys()):
            ...
            #vertexAttrib
            for k in list(typeData.vertxAttribArray.keys()):
            ...
            #primAttrib
            for k in list(typeData.primAttribArray.keys()):
            ...
            #groups prim
            for grp in list(typeData.primGroupsArray.keys()):
                array = []
                if primGroupOffset:
            ...
            #groups points
            for grp in list(typeData.pointGroupsArray.keys()):
                array = []
                #optimize array
        ...
        if self.pointGroupsArray:
            self.parent.msg2(2, 'Point groups:', ', '.join(sorted([x for x in list(self.pointGroupsArray.keys())])))
        if self.primGroupsArray:
            self.parent.msg2(2, 'Prim groups:', ', '.join(sorted([x for x in list(self.primGroupsArray.keys())])))
        if self.vertexAttributes:
            self.parent.msg2(2, 'Vertex attrib:', ', '.join(sorted([x for x in list(self.vertexAttributes.keys())])))
        if self.pointAttributes:
            self.parent.msg2(2, 'Point attrib:', ', '.join(sorted([x for x in list(self.pointAttributes.keys())])))
        if self.primAttributes:
            self.parent.msg2(2, 'Point attrib:', ', '.join(sorted([x for x in list(self.primAttributes.keys())])))
...
###### CONVERT DATA #################################################################
    ...
    def getAttribSummary(self):
        ...
        result = ''
        if self.pointAttributes:
            result += '   ' + str(len(self.pointAttributes)) + ' point attributes:\t' + ', '.join(list(self.pointAttributes.keys())) + '\n '
        if self.vertexAttributes:
            result += '   ' + str(len(self.vertexAttributes)) + ' vertex attributes:\t' + ', '.join(list(self.vertexAttributes.keys())) + '\n '
        if self.primAttributes:
            result += '   ' + str(len(self.primAttributes)) + ' primitive attributes:\t' + ', '.join(list(self.primAttributes.keys())) + '\n '
        return result
    ...
    def getAttributes(self):
        ...
        prim = []
        if self.pointAttributes:
            for name in list(self.pointAttributes.keys()):
        ...
        if self.vertexAttributes:
            for name in list(self.vertexAttributes.keys()):
    ...
    def createGlobalVariables(self):
        ...
        attrs = []
        count = 0
        keys = list(self.pointAttributes.keys()) + list(self.vertexAttributes.keys()) + list(self.primAttributes.keys())
        keys = list(set(keys))
        for k in keys:
            if not k in skip:
    ...
    def getPolyGroups(self):
        groups = []
        if self.primGroupsArray:
            for g in list(self.primGroupsArray.keys()):
    ...
    def getPointGroups(self):
        groups = []
        if self.pointGroupsArray:
            for g in list(self.pointGroupsArray.keys()):
