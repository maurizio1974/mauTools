...
def xgConvert(mesh, radius, jnts, dyn=None, mode=None):
    sel = cmds.ls(sl=True)
    if not sel:
    ...
    for s in sel:
        ...
        if jnts:
            ...
            if not cmds.objExists(grpJ):
                ...
                # cmds.addAttr(
                #     grpJ, ln='radius', at='double', min=0.001, max=1, dv=0.003, k=True)
                cmds.refresh()
            cmds.parent(hairJ, grpJ)
            for j in hairJ:
                cmds.setAttr(j + '.radius', 0.005)
                # cmds.connectAttr(grpJ + '.radius', j + '.radius')
        ...
        for z, h in enumerate(hair):
            ...
            if h != hair[0]:
                ...
                for x in range(0, vtx, 2):
                    ...
                    pos2 = cmds.pointPosition(h + '.vtx[' + str(x + 1) + ']')
                    avg = [
                        (pos1[0] + pos2[0]) / 2,
                        (pos1[1] + pos2[1]) / 2,
                        (pos1[2] + pos2[2]) / 2]
...
def curveOptions(hair, radius):
    # rmp = 'hairThickness_ramp'
    # if not cmds.objExists(rmp):
    #     cmds.shadingNode('ramp', asTexture=True, n=rmp)
    # cmds.setAttr(rmp + '.colorEntryList[0].color', 0.005, 0.005, 0.005, type='double3')
    # cmds.setAttr(rmp + '.colorEntryList[0].position', 0)
    # cmds.setAttr(rmp + '.colorEntryList[1].color', 0.0025, 0.0025, 0.0025, type='double3')
    ...
    for tr in hair:
        ...
        # cmds.connectAttr(rmp + '.outColorR', s + '.aiCurveWidth', f=True)
...
def convCmd(ui):
    ...
    jnts = cmds.checkBox(ui[4], q=True, v=True)
    if not grms:
        cmds.warning(
    ...
    if 'groom' not in val:
        ...
        exec(
            'import xgenToNhair;xgenToNhair.xgConvert("' + mesh + '", ' + str(radius) + ', ' + str(jnts) + ', ' + dict[val] + ')')
    else:
        jnt = 0
        if jnts:
            jnt = 1
...
def xUI():
    ...
    jTx = cmds.text('Make Guide Joints', p=frT)
    cbj = cmds.checkBox(l='', v=False)
    ...
    cmds.button(
        butD, e=True, c='xgenToNhair.convCmd(["' + mTx + '", "' + omM + '", "' + tsL + '", "' + rf + '", "' + cbj + '"])')
    # show window and size it
    cmds.showWindow(win)
    cmds.window(win, e=True, wh=[250, 350])
