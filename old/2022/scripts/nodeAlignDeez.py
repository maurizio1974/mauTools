...
class NodeAlignDeez(QtCore.QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.mayaWindow = shiboken.wrapInstance(long(mui.MQtUtil.mainWindow()), QtWidgets.QMainWindow)
    ...
    def _getItems(graphics):
        if not isinstance(graphics, QtWidgets.QGraphicsView):
            ...
            return [ item for item in graphics.items() if not isinstance(item, QtWidgets.QGraphicsPathItem) and not isinstance(item, QtWidgets.QGraphicsProxyWidget) and item.isSelected()
                   ]
