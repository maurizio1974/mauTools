...
def mirrorTrans(dir,t,r):
    ...
    while counter < half:
        ...
        if dir == 0:
            if t == 1:
                mc.move((sourcePos[0]*-1), sourcePos[1], sourcePos[2], destNode,absolute=True, worldSpace=True, rotatePivotRelative=True)
            if r == 1:
                mc.xform(destNode, ws = True, ro = (sourceRot[0], (sourceRot[1]*-1), (sourceRot[2]*-1)))
            print "Finished World XY Mirror"
            info = [newLoc[0],(sourcePos[0]*-1), sourcePos[1], sourcePos[2]]
        if dir == 1:
            if t == 1:
                mc.move( sourcePos[0], (sourcePos[1]*-1), sourcePos[2], destNode,absolute=True, worldSpace=True, rotatePivotRelative=True)
            if r == 1:
                mc.xform(destNode, ws = True, ro = (sourceRot[0], (sourceRot[1]*-1), ((sourceRot[2]*-1)+180)))
            print "Finished World XZ Mirror"
            info = [newLoc[0],sourcePos[0],(sourcePos[1]*-1), sourcePos[2]]
        if dir == 2:
            if t == 1:
                mc.move( sourcePos[0], sourcePos[1], (sourcePos[2]*-1), destNode,absolute=True, worldSpace=True, rotatePivotRelative=True)
            if r == 1:
                mc.xform(destNode, ws = True, ro = ((sourceRot[0]*-1), (sourceRot[1]*-1), sourceRot[2]))
...
def middleLoc():
    avgT = map(average, *[mc.pointPosition(p+".rotatePivot", world=True) for p in mc.ls(selection=True)])
    avgR = map(average, *[mc.xform(p, q = True, ro = True, ws = True) for p in mc.ls(selection=True)])
...
def mParentConstraint():
    ...
    mOutputMatrix = om.MTransformationMatrix(mEnd * mStart.inverse()).asMatrix()
