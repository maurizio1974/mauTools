...
class Menu(object):
    ...
    def show(cls, menu_type, menu_name, preferences, **kwargs):
        ...
        if not menu_name:
            return
        layout = cmds.layout(cmds.getPanel(up=True), q=True, p=True)
        if menu_type == cls.MEL:
            cls._mel(menu_name, preferences, p=layout, aob=True, mm=True, **kwargs)
        elif menu_type == cls.PYTHON:
            cls._python(menu_name, p=layout, aob=True, mm=True, **kwargs)
        else:
            cls._preset(menu_name, preferences, p=layout, aob=True, mm=True, **kwargs)
    ...
    @classmethod
    def _mel(cls, file_name, preferences, **kwargs):
        """show static marking menu"""
        if preferences["cache_mel"]:   # store file code into string
            if file_name not in cls._cache:
                with open("%s/menu_%s.mel" % (cmds.internalVar(umm=True), file_name)) as mel_file:
                    mel.eval("global proc %s(){%s}" % (file_name, mel_file.read()))
                cls._cache.add(file_name)
            cmds.popupMenu("mm_%i" % kwargs["b"], **kwargs)
            mel.eval(file_name)
        else:
            cmds.popupMenu("mm_%i" % kwargs["b"], **kwargs)
...
class UserData(object):
    ...
    def load(cls):
        ...
        try:  # check protocol version
            ...
            if data[0] != protocol:     # update data to current protocol
                # load preferences
                preferences_empty = DefaultData.preferences()
                preferences_empty.update(preferences)
                preferences = preferences_empty
                # load clusters
                cluster_empty = DefaultData.cluster()
                cluster_empty.update(cluster)
                for nodes in list(cluster_empty.values()):    # iterate through every node and update to current
                    for node, node_data in list(nodes.items()):
                        node_empty = DefaultData.node()
                        node_empty.update(node_data)
    ...
    def _import():
        ...
        def loadNode(is_panel=False, add_prefix=False):
            ...
            nodes = {}
            for i in range(ord(file_data.read(1))):
    ...
    def exporter(cls, *args):
        ...
        for directory in "/data", "/menus", "/commands":
            ...
            files = [f for f in os.listdir(full_dir) if os.path.isfile("%s/%s" % (full_dir, f))]
            for x in files:
...
class Engine(object):
    ...
    def pressSelected(self, **kwargs):
        ...
        if tool_node["menu"]:
            Menu.show(tool_node["menu_type"], tool_node["menu"], self.preferences,
        ...
        if self._last_selected["menu"]:
            Menu.show(self._last_selected["menu_type"], self._last_selected["menu"], self.preferences,
    ...
    def pressPreset(self, **kwargs):
        ...
        Menu.show(Menu.PRESET, self.cluster["preset"], self.preferences, b=Menu.MMB,
                  pmc=self._postMenu, **kwargs)
        Menu.show(self._last_preset["menu_type"], self._last_preset["menu"], self.preferences,
                  b=Menu.LMB, pmc=self._postMenu, **kwargs)
    ...
    def setPreset(self, preset):
        ...
        Menu.show(self._last_preset["menu_type"], self._last_preset["menu"], self.preferences,
                  b=Menu.LMB, pmc=self._postMenu, **self._preset_modifiers)
